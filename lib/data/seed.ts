import cetpsRaw from "@/data/cetps.json";
import industriesRaw from "@/data/industries.json";
import energyRaw from "@/data/energy.json";
import type {
  Cetp,
  Industry,
  FlowMeterReading,
...
export const cetps = cetpsRaw as Cetp[];
export const industries = industriesRaw as Industry[];
export const energy = energyRaw as EnergyData;
...
/* ------------------------------------------------------------------ */
/* CETP trends for charts                                              */
/* ------------------------------------------------------------------ */
export function buildCetpTrends(): CetpTrends[] {
  return cetps.map((cetp) => {
    const rnd = mulberry32(hashStr(cetp.id + "trend"));
    const wastewater: TrendPoint[] = WEEKS.map((w) => ({
      label: w,
      value: Math.round(cetp.treatedKLD * (0.82 + rnd() * 0.22)),
      capacity: cetp.capacityKLD,
    }));
    const compliance: TrendPoint[] = WEEKS.map((w, i) => ({
      label: w,
      value: Math.max(60, Math.min(99, Math.round(cetp.complianceScore - 6 + i * 0.6 + (rnd() - 0.5) * 6))),
    }));
    const flow: TrendPoint[] = WEEKS.map((w) => ({
      label: w,
      inlet: Math.round(cetp.capacityKLD * (0.9 + rnd() * 0.1)),
      recovered: Math.round(cetp.treatedKLD * cetp.recoveryRate * 0.01 * (0.92 + rnd() * 0.12)),
      value: Math.round(cetp.treatedKLD * (0.85 + rnd() * 0.15)),
    }));
